...
RSpec.describe "Admin::V1::SystemRequirement as :admin", type: :request do
  ...
  context "POST /system_requirements" do
    ...
    context "with valid params" do
      ...
      it 'returns last added SystemRequirement' do
        post url, headers: auth_header(user), params: system_requirement_params
        expected_system_requirements = SystemRequirement.last.as_json(only: %i(id name operational_system storage processor memory video_board))
        expect(body_json['system_requirement']).to eq expected_system_requirements
      end
  ...
  context "PATCH /system_requirements/:id" do
    ...
    context "with valid params" do
      ...
      it 'returns updates SystemRequirement' do
        patch url, headers: auth_header(user), params: system_requirement_params
        system_requirement.reload
        expected_system_requirement = system_requirement.as_json(only: %i(id memory name operational_system processor storage video_board))
  ...
  context "DELETE /system_requirements/:id" do
    ...
    context "without an associated system requirement" do

      it 'removes SystemRequirements' do
        expect do
